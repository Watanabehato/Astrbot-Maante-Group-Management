...
插件通过以下方式检测并阻止机器人之间的互相触发：

1. **检查发送者**：检查消息发送者是否是机器人账号
   - 检查配置的 `bot_qq_ids` 列表
   - 检查 event.sender 的 role 字段（某些实现会标记为 "bot"）
   - 检查发送者 QQ 号是否等于当前机器人的 self_id

2. **检查 @ 引用**：检查消息内容是否包含对机器人的 @ 提及
   - 检查 CQ 码格式：`[CQ:at,qq=机器人QQ]`
   - 检查纯文本格式：`@机器人QQ`
   - 检查 message_chain 中的 at 类型消息段

3. **检查引用回复**：检查消息是否引用回复了机器人的消息
   - 检查 event.reply 字段中的被引用消息发送者
   - 检查 message_chain 中的 reply 类型消息段
   - 检查 raw_event 中的 reply 信息
   - 防止通过 QQ 的"引用回复"功能强制唤醒机器人

**防止的场景：**
- ❌ 机器人 A 发送消息 → 机器人 B 响应
- ❌ 用户 @机器人A 发送消息 → 机器人 B 被强制唤醒响应
- ❌ 机器人 A @机器人B 发送消息 → 机器人 B 被 @ 唤醒
- ❌ 用户引用机器人 A 的消息发送 /gm 指令 → 机器人 B 被引用唤醒
